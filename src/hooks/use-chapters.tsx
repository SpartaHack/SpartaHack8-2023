...
const useChapters = (handleSourcing: (source: string) => void) => {
  ...
  useEffect(() => {
    const fetchChapters = async () => {
      if (learnContent) {
        const responseStream = await generateChapters((auth.currentUser?.uid!|| userId!), learnContent?.content_id!)
            ...
            chaptersArray.push({
              title: (
                <div className="flex flex-col">
                  <span className="text-sm font-extrabold hover:underline" onClick={() => handleSourcing(data.source)}>{formatTime(data.source)}</span>
                  <h3>{data && data?.response?.heading}</h3>
                </div>
              ),
              content: <p className="text-sm">{data && data?.response?.summary}</p>,
            });
          }
